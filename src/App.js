    ...
    axios
      ...
      .then((resp) => {
        this.setState({
          ...
          validStartPeriod: convertTimestamp(
            resp.data.items[0].valid_period.start
          ),
          validEndPeriod: convertTimestamp(resp.data.items[0].valid_period.end),
          forecastLastUpdated: convertTimestamp(
            resp.data.items[0].update_timestamp
          ),
          isFetched: true,
        });
      })
    ...
    return (
      <div className='container'>
        ...
        {this.state.isFetched ? (
          <ForecastPeriod
            forecastStart={this.state.validStartPeriod}
            forecastEnd={this.state.validEndPeriod}
          />
        ) : (
          <p>...</p>
        )}
        {this.state.isFetched ? (
          ...
          <h3>Fetching weather forecasts...</h3>
        )}
        <Footer
          isFetched={this.state.isFetched}
          forecastLastUpdated={this.state.forecastLastUpdated}
        />
      </div>
    );
  }
